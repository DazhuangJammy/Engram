---
name: engram
description: 默认对当前项目中的每一次用户请求、每一轮聊天和每一次回答前都先加载这个 skill，再判断是否需要调用 engram-server、是否需要匹配或加载某个专家、是否需要检查当前目录的专家包、以及是否要创建、优化、对话或写回 engram。尤其当用户希望得到专业回答、专家回答、更懂业务的回答、更像顾问的回答，或正在当前项目里使用 Claude Code、Codex、OpenClaw 时，都应优先触发并完整加载正文。
---
...
- 把当前项目视为一个完整的 Engram 路由与执行 skill。
- 每当用户发送请求、开始聊天、追问、继续任务，或希望获得更专业、更像专家、更像顾问的回答时，默认先完整加载这个 skill 正文，再决定是否调用 `engram-server` MCP 和哪个专家。
- 先检查当前会话里是否已经可以调用 `engram-server` MCP；如果还没有，就立刻执行 `cd "<skill-base>" && python3 scripts/bootstrap_mcp.py --project-root "<current-project-root>"`，自动把 `.mcp.json` 搭起来，再继续后续工作流。
- 强加载提问状态持久化在 `~/.engram/plugin-state.json`；一旦用户明确同意或拒绝，后续默认遵循该状态，不要反复追问，除非用户自己重置它。
- 一旦进入 Engram 工作流，完整遵循下方“完整路由策略”；不要删改、总结、折叠、抽样加载或仅引用文件名代替正文。
- 把当前仓库根目录视为 `<skill-base>`；需要理解实现时，优先查看 `<skill-base>/README.md`、`<skill-base>/CLAUDE参考.MD`、`<skill-base>/src/engram_server/server.py`、`<skill-base>/src/engram_server/loader.py`。

## Bootstrap

1. 每轮用户消息都先把这个 skill 视为默认入口，而不是等用户先说出“ngram”或“engram”。
2. 先检查当前宿主是否已经暴露 `engram-server` 相关 MCP 工具或 prompt。
3. 如果没有，就立即执行 `cd "<skill-base>" && python3 scripts/bootstrap_mcp.py --project-root "<current-project-root>"`；若已存在全局或项目级配置，就不要重复搭建。
4. MCP 接好后，立即执行 `cd "<skill-base>" && python3 scripts/bootstrap_state.py plugin-status` 读取强加载持久化状态。
5. 如果输出里的 `choice` 是 `unknown`，只在首次使用时问用户一次是否开启 `plugin` 强加载模式；用户同意时执行 `cd "<skill-base>" && python3 scripts/install_strong_load.py --host auto`，用户不同意时立刻执行 `cd "<skill-base>" && python3 scripts/bootstrap_state.py mark-plugin-declined`，以后不要再问，除非用户自己执行 `cd "<skill-base>" && python3 scripts/bootstrap_state.py reset-plugin-prompt` 或手动改 `~/.engram/plugin-state.json`。
6. 如果输出里的 `choice` 是 `accepted`，说明用户已经授权强加载；当前宿主若仍未启用 plugin，就直接执行 `cd "<skill-base>" && python3 scripts/install_strong_load.py --host auto`，不要再次询问。
7. 如果输出里的 `choice` 是 `declined`，不要再次询问；只需在合适时机简短告知：用户后续若想开启，可随时执行 `cd "<skill-base>" && python3 scripts/install_strong_load.py --host auto`。
8. 如果用户要创建 ngram、优化 ngram、与 ngram 对话、把内容写回 ngram、安装 engram、查看统计或打开 UI，优先路由到 `engram-server` 的现有能力。
9. 如果当前任务是在修改这个仓库本身，也仍然把下方的完整路由策略视为行为准则；涉及路由规则时，优先保持 `<skill-base>/CLAUDE参考.MD` 与实现一致。

## 完整路由策略（逐字镜像 `<skill-base>/CLAUDE参考.MD`）

下面区块必须与 `<skill-base>/CLAUDE参考.MD` 完全一致。执行时逐字遵循，不要删改、概括、抽样读取或跳过任何一行。

<!-- BEGIN CLAUDE_REFERENCE_MD -->
